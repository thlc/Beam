...
static BmString
BM_MAILKEY(const BmMailRef* ref)
{
...
\*------------------------------------------------------------------------------*/
BmRef<BmMail>
BmMail::CreateInstance(BmMailRef* ref)
{
	...
	BmRef<BmMail> mail(dynamic_cast<BmMail*>(BmRefObj::FetchObject(typeid(BmMail).name(), key)));
	if (mail)
		return mail;
	else
	  ...
	  mRatioSpam(BmMailRef::UNKNOWN_RATIO)
{
	...
	emptyMsg << "Content-Type: text/plain; charset=\"" << ThePrefs->GetString("DefaultCharset")
			 << "\"\r\n";
	emptyMsg << BM_FIELD_DATE << ": " << TimeToString(time(NULL), "%a, %d %b %Y %H:%M:%S %z");
	emptyMsg << "\r\n\r\n";
	SetTo(emptyMsg, "");
	if (outbound) {
		...
		if (identRef) {
			SetupFromIdentityAndRecvAddr(identRef.Get(), identRef->GetFromAddress());
		}
	}
}
...
{
	mOutbound = Status() == BM_MAIL_STATUS_DRAFT || Status() == BM_MAIL_STATUS_PENDING
...
\*------------------------------------------------------------------------------*/
void
BmMail::SetTo(const BmString& _text, const BmString account)
{
	BmString text;
...
BmMail::StartJob()
{
	...
	try {
		...
		BM_LOG2(BM_LogMailParse, BmString("opening mail-file <") << eref.name << ">");
		for (int i = 0; (err = mailFile.SetTo(&eref, B_READ_ONLY)) == B_BUSY; ++i) {
			if (i == 100)
				throw BM_runtime_error(BmString("Node is locked too long for "
												"mail-file <")
									   << eref.name << "> \n\nError:" << strerror(err));
			BM_LOG2(BM_LogMailTracking, BmString("Node is locked for mail-file <")
											<< eref.name << ">. We take a nap and try again...");
			snooze(200 * 1000);
		}
		if (err != B_OK) {
		...
		if ((err = mailFile.GetSize(&mailSize)) != B_OK)
			BM_THROW_RUNTIME(BmString("Could not get size of mail-file <")
							 << eref.name << "> \n\nError:" << strerror(err));
		BM_LOG2(BM_LogMailParse, BmString("...should be reading ") << mailSize << " bytes");
		...
		for (int32 offs = 0; (skipChecks || ShouldContinue()) && offs < mailSize;) {
			...
			ssize_t read = mailFile.Read(
				pos, mailSize - offs < (off_t)blocksize ? size_t(mailSize - offs) : blocksize);
			BM_LOG3(BM_LogMailParse, BmString("...read a block of ") << read << " bytes");
			if (read < 0)
				throw BM_runtime_error(BmString("Could not fetch mail from "
												"file\n\t<")
									   << eref.name << ">\n\n Result: " << strerror((int32)read));
			if (!read)
				break;
			realSize += read;
			offs += (int32)read;
		...
		if (!skipChecks && !ShouldContinue())
			return false;
		BM_LOG2(BM_LogMailParse, BmString("...real size is ") << realSize << " bytes");
		if (realSize > mailSize)
			throw BM_runtime_error(BmString("Real size is ")
								   << realSize << " bytes but expected size was only " << mailSize
								   << " bytes!?!");
		buf[realSize] = '\0';
...
BmMail::Store()
{
	...
	try {
		...
		if (mMailRef && mMailRef->InitCheck() == B_OK) {
			...
			if (mEntry.InitCheck() != B_OK) {
				if ((err = mEntry.SetTo(mMailRef->EntryRefPtr())) != B_OK)
					BM_THROW_RUNTIME(BmString("Could not create entry from mail-ref <")
									 << mMailRef->Key() << ">\n\n Result: " << strerror(err));
			}
		...
		if ((err = destDir.InitCheck()) != B_OK)
			BM_THROW_RUNTIME(BmString("Could not set up a directory for mail <")
							 << filename << ">.\n\n Result: " << strerror(err));
		StoreIntoFile(&destDir, filename, status, whenCreated, &backupEntry);
		// ...and fetch resulting entry-ref
		entry_ref eref;
		if ((err = mEntry.GetRef(&eref)) != B_OK) {
			BM_THROW_RUNTIME(BmString("Could not get entry-ref for mail <")
							 << filename << ">.\n\n Result: " << strerror(err));
		}
...
\*------------------------------------------------------------------------------*/
void
BmMail::StoreIntoFile(BDirectory* destDir, BmString filename, const BmString& status,
	bigtime_t whenCreated, BEntry* backupEntry)
{
	...
	if ((err = mEntry.SetTo(destDir, filename.String())) != B_OK)
		BM_THROW_RUNTIME(BmString("Could not create entry for mail-file <")
						 << filename << ">\n\n Result: " << strerror(err));
		...
		if (mEntry.Exists()) {
			...
			while (mEntry.Exists()) {
				if (i > 1000000) {
					...
					BM_THROW_RUNTIME(
						BmString("Unable to find unique name for mail-file <") << fn << ">!");
				}
	...
	if (err != B_OK)
		BM_THROW_RUNTIME(BmString("Could not create backed mail-file\n\t<")
						 << filename << ">\n\n Result: " << strerror(err));
...
\*------------------------------------------------------------------------------*/
void
BmMail::StoreAttributes(BNode& mailNode, const BmString& status, bigtime_t whenCreated)
{
	//
	mailNode.WriteAttr(BM_MAIL_ATTR_STATUS, B_STRING_TYPE, 0, status.String(), status.Length() + 1);
	mailNode.WriteAttr(
		BM_MAIL_ATTR_ACCOUNT, B_STRING_TYPE, 0, mAccountName.String(), mAccountName.Length() + 1);
	mailNode.WriteAttr(BM_MAIL_ATTR_IDENTITY, B_STRING_TYPE, 0, mIdentityName.String(),
	...
	if (mOutbound) {
		...
		if (status == BM_MAIL_STATUS_PENDING)
			...
			flags = B_MAIL_SENT;
		mailNode.WriteAttr(BM_MAIL_ATTR_FLAGS, B_INT32_TYPE, 0, &flags, sizeof(int32));
	}
	//
	int32 headerLength = HeaderLength();
	int32 contentLength = MAX(0, mText.Length() - headerLength);

	mailNode.WriteAttr(BM_MAIL_ATTR_HEADER, B_INT32_TYPE, 0, &headerLength, sizeof(int32));
	mailNode.WriteAttr(BM_MAIL_ATTR_CONTENT, B_INT32_TYPE, 0, &contentLength, sizeof(int32));
	//
	int32 hasAttachments = HasAttachments();
	mailNode.WriteAttr(
		BM_MAIL_ATTR_ATTACHMENTS, B_INT32_TYPE, 0, &hasAttachments, sizeof(hasAttachments));
	//
	mailNode.WriteAttr(BM_MAIL_ATTR_MARGIN, B_INT32_TYPE, 0, &mRightMargin, sizeof(int32));
	//
	mailNode.WriteAttr(
		BM_MAIL_ATTR_WHEN_CREATED, B_UINT64_TYPE, 0, &whenCreated, sizeof(whenCreated));
	//
	...
	if (mRatioSpam != BmMailRef::UNKNOWN_RATIO)
		mailNode.WriteAttr(
			BM_MAIL_ATTR_RATIO_SPAM, B_FLOAT_TYPE, 0, &mRatioSpam, sizeof(mRatioSpam));
}
...
\*------------------------------------------------------------------------------*/
bool
BmMail::SetDestFolderName(const BmString& destFolderName)
{
...
BmMail::DestFolder() const
{
		...
		while (!folder) {
			...
			if (!folder) {
				...
				if (BmMailFolder::IsSystemFolderSubPath(mDestFolderName)) {
					...
					BmString fullPath = ThePrefs->GetString("MailboxPath") + "/" + mDestFolderName;
	...
	while (!folder) {
		...
		if (mMailRef && mMailRef->InitCheck() == B_OK) {
			...
			BmString folderName
				= mOutbound ? BmMailFolder::OUT_FOLDER_NAME : BmMailFolder::IN_FOLDER_NAME;
	...
	if (mDestFolderName.Length()) {
		if (mEntry.InitCheck() == B_OK || (mMailRef && mMailRef->InitCheck() == B_OK)) {
			...
			if ((err = mEntry.MoveTo(&newHomeDir)) != B_OK) {
				BM_LOGERR(BmString("Could not move mail <")
						  << Name() << "> to folder <" << destFolder->DisplayKey()
						  << ">\n\n Result: " << strerror(err));
				return false;
			}
			entry_ref eref;
			if ((err = mEntry.GetRef(&eref)) != B_OK) {
				BM_LOGERR(BmString("Could not get entry-ref for mail <")
						  << Name() << ">.\n\n Result: " << strerror(err));
				return false;
			}
			// create a (new) mail-ref for the freshly saved mail:
...
BmMail::Send(bool now)
{
	...
	if (now) {
		BmRef<BmListModelItem> smtpRef = TheSmtpAccountList->FindItemByKey(AccountName());
...
\*------------------------------------------------------------------------------*/
void
BmMail::AddAttachmentFromRef(const entry_ref* ref, const BmString& charset)
{
	if (mBody)
...
\*------------------------------------------------------------------------------*/
bool
BmMail::ReconstructRawText()
{
	BmRef<BmBodyPart> bodyPart(mBody->EditableTextBody());
	return ConstructRawText(
		bodyPart ? bodyPart->DecodedData() : BM_DEFAULT_STRING, DefaultCharset(), mAccountName);
}
...
\*------------------------------------------------------------------------------*/
bool
BmMail::ConstructRawText(
	const BmString& editedUtf8Text, const BmString& charset, const BmString smtpAccount)
{
...
\*------------------------------------------------------------------------------*/
void
BmMail::SetBaseMailInfo(BmMailRef* ref, const BmString newStatus)
{
...
\*------------------------------------------------------------------------------*/
void
BmMail::AddBaseMailRef(BmMailRef* ref)
{
	mBaseRefVect.push_back(ref);
}
...
\*------------------------------------------------------------------------------*/
void
BmMail::SetDefaultHeaders(const BmString& defaultHeaders)
{
	if (mHeader) {
...
\*------------------------------------------------------------------------------*/
void
BmMail::SetNewHeader(const BmString& headerStr)
{
	BmString newMsgText;
...
\*------------------------------------------------------------------------------*/
int32
BmMail::HeaderLength() const
{
	return mHeader ? mHeader->HeaderLength() : 0;
}
...
\*------------------------------------------------------------------------------*/
const BmString&
BmMail::HeaderText() const
{
	return mHeader ? mHeader->HeaderString() : BM_DEFAULT_STRING;
}
...
\*------------------------------------------------------------------------------*/
const BmString&
BmMail::GetFieldVal(const BmString fieldName)
{
	if (mHeader)
...
\*------------------------------------------------------------------------------*/
void
BmMail::SetFieldVal(const BmString fieldName, const BmString value)
{
...
\*------------------------------------------------------------------------------*/
void
BmMail::RemoveField(const BmString fieldName)
{
	mHeader->RemoveField(fieldName);
}
...
\*------------------------------------------------------------------------------*/
bool
BmMail::IsFieldEmpty(const BmString fieldName)
{
	return mHeader ? mHeader->IsFieldEmpty(fieldName) : true;
}
...
\*------------------------------------------------------------------------------*/
bool
BmMail::IsRedirect() const
{
	return mHeader ? mHeader->IsRedirect() : false;
}
...
\*------------------------------------------------------------------------------*/
void
BmMail::SetupFromIdentityAndRecvAddr(BmIdentity* ident, const BmString& recvAddr)
{
	if (ident && recvAddr.Length()) {
		BmString realName = ident->RealName();
		BmString fromAddress;
		if (recvAddr.FindFirst('<') < 0 && realName.Length()) {
			fromAddress = BmAddress::QuotedPhrase(realName) + " <" + recvAddr + ">";
		} else
...
\*------------------------------------------------------------------------------*/
void
BmMail::DetermineRecvAddrAndIdentity(BmString& receivingAddr, BmRef<BmIdentity>& identRefOut)
{
	...
	BmRef<BmIdentity> identRef
		= dynamic_cast<BmIdentity*>(TheIdentityList->FindItemByKey(IdentityName()).Get());
	if (identRef) {
		...
		// deleted by now):
		TheIdentityList->FindIdentitiesForRecvAccount(AccountName(), identities);
	}

	// now find the receiving address by iterating over all likely identites
	receivingAddr = Header()->DetermineReceivingAddrFor(identities, &identRefOut);
...
\*------------------------------------------------------------------------------*/
void
BmMail::SetSignatureByName(const BmString sigName)
{
...
\*------------------------------------------------------------------------------*/
void
BmMail::MarkAs(const char* status)
{
	if (InitCheck() != B_OK)
		return;
	try {
...
\*------------------------------------------------------------------------------*/
const BmString&
BmMail::DefaultStatus() const
{
	if (!mDefaultStatus.Length())
		mDefaultStatus = mOutbound ? BM_MAIL_STATUS_DRAFT : BM_MAIL_STATUS_NEW;
...
\*------------------------------------------------------------------------------*/
bool
BmMail::HasBeenClassified() const
{
...
\*------------------------------------------------------------------------------*/
bool
BmMail::IsMarkedAsSpam() const
{
	return mMailRef ? mMailRef->Classification() == BM_MAIL_CLASS_SPAM
					: mClassification == BM_MAIL_CLASS_SPAM;
}
...
\*------------------------------------------------------------------------------*/
bool
BmMail::IsMarkedAsTofu() const
{
	return mMailRef ? mMailRef->Classification() == BM_MAIL_CLASS_TOFU
					: mClassification == BM_MAIL_CLASS_TOFU;
}
