...
public:
	...
	static BmRef<BmMailRef> CreateInstance(entry_ref& eref, struct stat* st = NULL);
	...
	inline const BmString& Classification() const { return mClassification; }
	inline float RatioSpam() const { return mRatioSpam; }
	inline const BmString& RatioSpamString() const { return mRatioSpamString; }

	// setters:
	inline void EntryRef(entry_ref& e) { mEntryRef = e; }
	inline void WhenCreated(const bigtime_t& t) { mWhenCreated = t; }
	inline void Classification(const BmString& c) { mClassification = c; }
