...
BmMailFilter::BmMailFilter(
	const BmString& name, BmFilter* filter, bool executeInMem, bool needControllers)
	: BmJobModel(name),
	  mFilter(filter),
	  mMailRefs(NULL),
...
\*------------------------------------------------------------------------------*/
void
BmMailFilter::SetMailRefVect(BmMailRefVect* refVect)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFilter::AddMail(BmMail* mail)
{
	if (mail)
...
\*------------------------------------------------------------------------------*/
bool
BmMailFilter::ShouldContinue()
{
	return inherited::ShouldContinue() || !NeedControllersToContinue();
}
...
BmMailFilter::StartJob()
{
	try {
		...
		for (uint32 i = 0; ShouldContinue() && i < mMails.size(); ++i) {
			...
			BmString currentCount = BmString() << ++c << " of " << count;
			UpdateStatus(delta, mMails[i]->Name().String(), currentCount.String());
		}
		mMails.clear();
		BmString currentCount = BmString() << c << " of " << count;
...
\*------------------------------------------------------------------------------*/
void
BmMailFilter::Execute(BmMail* mail)
{
	...
	if (!mFilter) {
		BM_LOG2(BM_LogFilter, BmString("Searching filter-chain for mail with Id <")
								  << mail->Name() << ">...");
		...
		if (mail->Outbound()) {
			...
			if (recvAcc)
				chainItem = TheFilterChainList->FindItemByKey(recvAcc->FilterChain());
			else
		...
		if (chain) {
			BM_LOG2(BM_LogFilter, BmString("...found chain ")
									  << chain->DisplayKey() << ", applying all its filters...");
...
\*------------------------------------------------------------------------------*/
bool
BmMailFilter::ExecuteFilter(BmMail* mail, BmFilter* filter, BmMsgContext* msgContext)
{
	if (filter->IsDisabled()) {
		BM_LOG2(BM_LogFilter, BmString("Addon for Filter ")
								  << filter->Name() << " (type=" << filter->Kind()
								  << ") has not been loaded, we skip this filter.");
	} else {
		BM_LOG2(BM_LogFilter, BmString("Executing Filter ")
								  << filter->Name() << " (type=" << filter->Kind() << ")...");
		msgContext->ResetChanges();
		filter->Execute(msgContext);
		if (msgContext->FieldHasChanged("Identity")) {
			BmString newIdentity = msgContext->GetString("Identity");
			BM_LOG(BM_LogFilter, BmString("Filter ")
									 << filter->Name() << ": setting identity to " << newIdentity);
		}
		if (msgContext->FieldHasChanged("Status")) {
			BmString newStatus = msgContext->GetString("Status");
			BM_LOG(BM_LogFilter, BmString("Filter ")
									 << filter->Name() << ": setting status to " << newStatus);
		}
		if (msgContext->FieldHasChanged("MoveToTrash") && msgContext->GetBool("MoveToTrash")) {
			BM_LOG(
				BM_LogFilter, BmString("Filter ") << filter->Name() << ": moving mail to trash.");
		}
		if (msgContext->FieldHasChanged("RejectMsg")) {
			BmString rejectMsg = msgContext->GetString("RejectMsg");
			BM_LOG(BM_LogFilter,
				BmString("Filter ") << filter->Name() << ": rejecting mail with msg " << rejectMsg);
		}
		if (msgContext->FieldHasChanged("ListId")) {
			BmString newListId = msgContext->GetString("ListId");
			BM_LOG(BM_LogFilter, BmString("Filter ")
									 << filter->Name() << ": setting ListId to " << newListId);
		}
		bool stopProcessing = msgContext->GetBool("StopProcessing");
		if (stopProcessing) {
			BM_LOG(BM_LogFilter, BmString("Filter ")
									 << filter->Name() << ": wants to stop processing this chain.");
			return false;
		}
	}
	return true;
...
\*------------------------------------------------------------------------------*/
void
BmMailFilter::UpdateStatus(const float delta, const char* filename, const char* currentCount)
{
