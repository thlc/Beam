...
public:
	BmSlaveHandler();
	~BmSlaveHandler();
	void Run(const char* name, thread_func func, BMessage* msg, uint16 minCountForThread = 10);
...
void
BmSlaveHandler::Run(const char* name, thread_func func, BMessage* msg, uint16 minCountForThread)
{
	...
	if (!refVect || refVect->size() <= minCountForThread) {
		...
		BmString tname(name);
		tname << "(" << mSlaveNum++ << ")";
		thread_id t_id = spawn_thread(func, tname.String(), B_NORMAL_PRIORITY, msg);
		if (t_id < 0)
...
MarkMailsAs(void* data)
{
	...
	if (msgCount > 1) {
		...
		if (newStatus == BM_MAIL_STATUS_READ) {
			...
			if (hasAdvanced) {
				BmString s(
					"Some of the selected messages are not new."
					"\n\nShould Beam mark only the new messages "
					"as being read?");
				BAlert* alert = new BAlert("Set mail status", s.String(), "Cancel",
...
TrashMails(void* data)
{
	...
	if (refVect)
		countFound = refVect->size();
	if (countFound > 0) {
		BM_LOG(BM_LogApp, BmString("Asked to trash ") << countFound << " mails");
		struct entry_ref* refs = new entry_ref[countFound];
...
\*------------------------------------------------------------------------------*/
static void
CreateMailsWithFactory(BMessage* msg, BmMailRefVect* refVect, BmMailFactory* factory)
{
...
EditMailsAsNew(void* data)
{
	...
	if (refVect)
		msgCount = refVect->size();
	if (msgCount) {
		BM_LOG(BM_LogApp, BmString("Asked to edit ") << msgCount << " mails as new.");
...
RedirectMails(void* data)
{
	...
	if (refVect)
		msgCount = refVect->size();
	if (msgCount) {
		BM_LOG(BM_LogApp, BmString("Asked to redirect ") << msgCount << " mails.");
...
ForwardMails(void* data)
{
	...
	if (msgCount > 1) {
		...
		BmString s(
			"You have selected more than one message.\n\n"
		...
		BAlert* alert = new BAlert("Forwarding multiple mails", s.String(), "Cancel",
			"Keep separate", "Join", B_WIDTH_AS_USUAL, B_OFFSET_SPACING, B_IDEA_ALERT);
		alert->SetShortcut(0, B_ESCAPE);
		buttonPressed = alert->Go();
	}
	if (buttonPressed > 0) {
		...
		BmString selectedText = msg->FindString(BeamApplication::MSG_SELECTED_TEXT);
...
ReplyToMails(void* data)
{
	...
	if (msgCount > 1) {
		...
		BmString s(
			...
			"to keep the messages separate?");
		BAlert* alert = new BAlert("Replying to multiple mails", s.String(), "Keep separate",
			"Join per recipient", "Join all", B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_IDEA_ALERT);
		buttonPressed = alert->Go();
	}
	if (buttonPressed > -1) {
	...
	if (msgCount) {
		...
		if (result == B_OK) {
			...
			for (uint32 mailIdx = 0;
				...
				BPoint topOfLine
					= mailView->PointAt(mailView->OffsetAt(BPoint(5, currFrame.bottom)));
	  ...
	  mDeskbarItemIsOurs(false)
{
	...
	try {
		...
		ColumnListView::SetExtendedSelectionPolicy(ThePrefs->GetBool("ListviewLikeTracker", false));
		...
		TheRecvAccountList->AddForeignKey(BmIdentity::MSG_RECV_ACCOUNT, TheIdentityList.Get());
		TheSmtpAccountList->AddForeignKey(BmIdentity::MSG_SMTP_ACCOUNT, TheIdentityList.Get());
		TheSignatureList->AddForeignKey(BmIdentity::MSG_SIGNATURE_NAME, TheIdentityList.Get());
		TheFilterChainList->AddForeignKey(
			BmRecvAccount::MSG_FILTER_CHAIN, TheRecvAccountList.Get());
		TheRecvAccountList->AddForeignKey(BmSmtpAccount::MSG_ACC_FOR_SAP, TheSmtpAccountList.Get());
		TheMailFolderList->AddForeignKey(BmRecvAccount::MSG_HOME_FOLDER, TheRecvAccountList.Get());
		TheMailFolderList->AddForeignKey(BmFilterAddon::FK_FOLDER, TheFilterList.Get());
		TheIdentityList->AddForeignKey(BmFilterAddon::FK_IDENTITY, TheFilterList.Get());
...
\*------------------------------------------------------------------------------*/
void
BeamApplication::AppActivated(bool active)
{
}
...
\*------------------------------------------------------------------------------*/
void
BeamApplication::RefsReceived(BMessage* msg)
{
	if (!msg)
		return;
	entry_ref inref;
	...
	for (int index = 0; msg->FindRef("refs", index, &inref) == B_OK; ++index) {
		if (entry.SetTo(&inref, true) != B_OK || entry.GetRef(&eref) != B_OK)
			continue;
...
\*------------------------------------------------------------------------------*/
void
BeamApplication::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_JOBWIN_POP:
			...
			case BMM_CHECK_MAIL:
			{
				...
				if (key) {
					bool isAutoCheck = msg->FindBool(BmRecvAccountList::MSG_AUTOCHECK);
					BM_LOG(BM_LogApp, BmString("RecvAccount ")
										  << key << " asks to check mail "
										  << (isAutoCheck ? "(auto)" : "(manual)"));
					if (!isAutoCheck || !ThePrefs->GetBool("AutoCheckOnlyIfPPPRunning", true)
						|| IsPPPRunning()) {
						BM_LOG(
							BM_LogApp, BmString("RecvAccount ") << key << ": mail is checked now");
						TheRecvAccountList->CheckMailFor(key, isAutoCheck);
						if (!isAutoCheck && ThePrefs->GetBool("SendPendingMailsOnCheck", true))
			...
			case BMM_CREATE_PERSON_FROM_ADDR:
			{
				...
				for (idx = 0; msg->FindString("name", idx, &name) == B_OK;) {
					...
					BmRef<BmPerson> person = ThePeopleList->FindPersonByName(name);
					if (person) {
			...
			case BMM_EDIT_PERSON_WITH_ADDR:
			{
				...
				for (idx = 0; msg->FindString("email", idx, &email) == B_OK;) {
					BmRef<BmPerson> person = ThePeopleList->FindPersonByEmail(email);
					if (person) {
						eref = person->EntryRef();
						be_roster->Launch(&eref);
					}
...
BeamApplication::LaunchURL(const BmString url)
{
	...
	if (!(result == B_OK || result == B_ALREADY_RUNNING)) {
		(new BAlert("",
			 (BmString("Could not launch application for url\n\t")
				 << url << "\n\nError:\n\t" << strerror(result))
				 .String(),
			 "OK"))
			->Go();
...
{
	ImageAboutWindow* aboutWin
		= new ImageAboutWindow("About Beam", "Beam", TheResources->IconByName("AboutIcon")->bitmap,
			15, "BEware, Another Mailer\n(c) Oliver Tappe, Berlin, Germany", "",
...
\*------------------------------------------------------------------------------*/
bool
BeamApplication::HandlesMimetype(const BmString mimetype)
{
