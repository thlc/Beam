...
\*------------------------------------------------------------------------------*/
const char*
BmDateWidthAdjuster::operator()(int32 colIdx, time_t utc)
{
	...
	if (column->Width() != cachedWidth) {
		...
		if (ThePrefs->GetBool("UseSwatchTimeInRefView", false)) {
			const char* formats[] = {
				"%A, %Y-%m-%d @", "%a, %Y-%m-%d @", "%Y-%m-%d @", "%y-%m-%d @", "%y-%m-%d", NULL};
			...
			const char* formats[] = {"%A, %Y-%m-%d %H:%M:%S", "%a, %Y-%m-%d %H:%M:%S",
				"%a, %Y-%m-%d %H:%M", "%Y-%m-%d %H:%M", "%y-%m-%d %H:%M", "%y-%m-%d", NULL};
			for (const char** f = formats; *f; ++f) {
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmMailRefItem::BmMailRefItem(ColumnListView* lv, BmListModelItem* _item)
	: inherited(lv, _item),
...
\*------------------------------------------------------------------------------*/
void
BmMailRefItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
...
\*------------------------------------------------------------------------------*/
const int32
BmMailRefItem::GetNumValueForColumn(int32 column_index) const
{
	BmMailRef* ref(ModelItem());
	if (column_index == COL_STATUS_I || column_index == COL_STATUS) {
		...
		return st == BM_MAIL_STATUS_NEW			 ? 0
			   ...
			   : st == BM_MAIL_STATUS_REDIRECTED ? 7
												 : 99;
...
\*------------------------------------------------------------------------------*/
const time_t
BmMailRefItem::GetDateValueForColumn(int32 column_index) const
{
	BmMailRef* ref(ModelItem());
...
\*------------------------------------------------------------------------------*/
const bigtime_t
BmMailRefItem::GetBigtimeValueForColumn(int32 column_index) const
{
	BmMailRef* ref(ModelItem());
...
\*------------------------------------------------------------------------------*/
const char*
BmMailRefItem::GetUserText(int32 colIdx, float colWidth) const
{
	BmMailRef* ref(ModelItem());
	if (!ref)
		return "";
	...
	switch (colIdx) {
		...
		{
			text = mWhenCreatedStringAdjuster(colIdx, time_t(ref->WhenCreated() / (1000 * 1000)));
			break;
		}
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::ReselectionInfo ::NoteDeselectionOf(const entry_ref& eref)
{
	mEntryRef = eref;
...
\*------------------------------------------------------------------------------*/
bool
BmMailRefView::ReselectionInfo ::QualifiesForReselection(const entry_ref& eref)
{
	const int secs = 2;
	return mEntryRef == eref && system_time() <= mTimeOfDeselection + secs * 1000 * 1000;
}
...
\*------------------------------------------------------------------------------*/
BmMailRefView*
BmMailRefView::CreateInstance(int32 width, int32 height)
{
	  ...
	  mHiddenState(BMH_NO_INIT)
{
	...
	AddColumn(new CLVColumn("P", 18.0,
		flags | CLV_NOT_RESIZABLE | CLV_COLDATA_NUMBER | CLV_COLTYPE_BITMAP, 18.0,
		"(P)riority [icon]"));
	AddColumn(new CLVColumn("From", 150.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Subject", 300.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Date", 100.0, flags | CLV_COLDATA_DATE | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Size", 60.0,
		flags | CLV_COLDATA_NUMBER | CLV_RIGHT_JUSTIFIED | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Cc", 100.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Account", 120.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("To", 150.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Reply to", 150.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Name", 150.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn(
		"Delivery date", 110.0, flags | CLV_COLDATA_BIGTIME | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("Tracker name", 150.0, flags | CLV_COLTYPE_USERTEXT, 20.0));
	AddColumn(new CLVColumn("S", 100.0, flags | CLV_COLTYPE_USERTEXT, 40.0, "(S)tatus [text]"));
	AddColumn(new CLVColumn("A", 100.0, flags | CLV_COLDATA_NUMBER | CLV_COLTYPE_USERTEXT, 18.0,
		"(A)ttachments [text]"));
	AddColumn(new CLVColumn(
		"P", 100.0, flags | CLV_COLDATA_NUMBER | CLV_COLTYPE_USERTEXT, 18.0, "(P)riority [text]"));
	AddColumn(new CLVColumn("Identity", 120.0, flags | CLV_COLTYPE_USERTEXT, 40.0));
	AddColumn(new CLVColumn("Class", 40.0, flags | CLV_COLTYPE_USERTEXT, 40.0));
	AddColumn(new CLVColumn("RatioSpam", 100.0,
		flags | CLV_COLDATA_NUMBER | CLV_RIGHT_JUSTIFIED | CLV_COLTYPE_USERTEXT, 40.0));
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmMailRefView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::TeamUpWith(BmMailRefFilterControl* fc)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::TeamUpWith(BmMailRefViewFilterControl* fc)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::ColumnWidthChanged(int32 colIdx, float NewWidth)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::AttachedToWindow()
{
	...
	if (lockedIconHandle && unlockedIconHandle) {
		...
		BPicture* lockedPicture = TheResources->CreatePictureFor(locked, B_V_SCROLL_BAR_WIDTH + 1,
			labelViewHeight, false, B_PANEL_BACKGROUND_COLOR, BmPicFrame_ActionButton);
		BPicture* unlockedPicture
			= TheResources->CreatePictureFor(unlocked, B_V_SCROLL_BAR_WIDTH + 1, labelViewHeight,
				false, B_PANEL_BACKGROUND_COLOR, BmPicFrame_ActionButton);
		...
		if (sv) {
			...
			TheBubbleHelper->SetHelp(mLockLabelsButton, "Click to lock/unlock column layout");
		}
	}
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case B_TRASH_TARGET:
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::TrashSelectedMessages()
{
	BList selectedItems;
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::KeyDown(const char* bytes, int32 numBytes)
{
	if (numBytes == 1) {
		switch (bytes[0]) {
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::MouseDown(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailRefView::InitiateDrag(BPoint, int32 index, bool wasSelected)
{
	if (!wasSelected)
		return false;
	...
	for (int32 i = 0; (currIdx = CurrentSelection(i)) >= 0; ++i) {
		...
		if (i % 100 == 0)
			BM_LOG2(BM_LogGui, BmString("MailRefView::InitiateDrag() - processing ")
								   << i << "th selection");
	}
	vector<int> cols;
	cols.push_back(0);
...
\*------------------------------------------------------------------------------*/
bool
BmMailRefView::AcceptsDropOf(const BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::HandleDrop(BMessage* msg)
{
	type_code tc;
	int32 refCount;
	if (mCurrFolder && msg && msg->what == B_SIMPLE_DATA
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::WindowActivated(bool active)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::FrameResized(float width, float height)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::ShowFolder(BmMailFolder* folder)
{
	try {
		StopJob();
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::JobIsDone(bool completed)
{
	if (IsHidden())
	...
	if (completed && mCurrFolder) {
		...
		if (refList) {
			if (mPartnerFilterControl) {
				BmString label = refList->Filter() ? refList->Filter()->Label()
												   : BmMailRefFilterControl::TIME_SPAN_NONE;
				mPartnerFilterControl->MarkItem(label.String());
			}
			BmRef<BmListModelItem> refItem = refList->FindItemByKey(mCurrFolder->SelectedRefKey());
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmMailRefView::AddModelItem(BmListModelItem* item)
{
	BmListViewItem* viewItem = inherited::AddModelItem(item);
	BmMailRef* mailRef = dynamic_cast<BmMailRef*>(item);
	if (mailRef && mReselectionInfo.QualifiesForReselection(mailRef->EntryRef())) {
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::RemoveModelItem(BmListModelItem* item)
{
	BmMailRef* mailRef = dynamic_cast<BmMailRef*>(item);
	if (mailRef) {
		int32 selectedIdx = 0;
		...
		while ((selectedIdx = CurrentSelection(idx++)) >= 0) {
			BmListViewItem* viewItem = dynamic_cast<BmListViewItem*>(ItemAt(selectedIdx));
...
\*------------------------------------------------------------------------------*/
BmString
BmMailRefView::StateInfoBasename()
{
	return "MailRefView";
}
...
\*------------------------------------------------------------------------------*/
BmString
BmMailRefView::StateInfoFilename(bool forRead)
{
	if (mCurrFolder) {
		...
		while (folderRef) {
			...
			if (folder) {
				stateInfoFilename = StateInfoBasename()
									<< "_" << folder->Key() << " (" << folder->Name() << ")";
				if (!folder->RefListStateInfoConnectedToParent()) {
					BEntry stateInfoEntry(
						BeamRoster->StateInfoFolder(), stateInfoFilename.String());
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::AddSelectedRefsToMsg(BMessage* msg, BList* itemList)
{
	if (mPartnerMailView) {
		...
		if (selectedText.Length())
			msg->AddString(BeamApplication::MSG_SELECTED_TEXT, selectedText.String());
	}
	...
	if (CurrentSelection(0) >= 0) {
		...
		msg->AddPointer(BeamApplication::MSG_MAILREF_VECT, static_cast<void*>(refs));
	}
}
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::SelectionChanged(void)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::SendNoticesIfNeeded(bool haveSelectedRef)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::ItemInvoked(int32 index)
{
	BmMailRefItem* refItem;
	...
	if (refItem) {
		BmMailRef* ref(refItem->ModelItem());
		if (ref) {
			if (ref->Status() == BM_MAIL_STATUS_DRAFT || ref->Status() == BM_MAIL_STATUS_PENDING) {
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::AddMailRefMenu(BMenu* menu, BHandler* target, bool isContextMenu)
{
	if (!menu)
		return;
	...
	AddItemToMenu(replyMenu, CreateMenuItem("Reply (auto)", BMM_REPLY), target);
	AddItemToMenu(replyMenu, CreateMenuItem("Reply to list", BMM_REPLY_LIST), target);
	AddItemToMenu(replyMenu, CreateMenuItem("Reply to person", BMM_REPLY_ORIGINATOR), target);
	AddItemToMenu(replyMenu, CreateMenuItem("Reply to all", BMM_REPLY_ALL), target);
	...
	AddItemToMenu(fwdMenu, CreateMenuItem("Forward inline", BMM_FORWARD_INLINE), target);
	AddItemToMenu(fwdMenu, CreateMenuItem("Forward as attachment", BMM_FORWARD_ATTACHED), target);
	AddItemToMenu(fwdMenu,
		CreateMenuItem("Forward inline (with attachments)", BMM_FORWARD_INLINE_ATTACH), target);
	AddItemToMenu(menu, CreateMenuItem("Redirect", BMM_REDIRECT), target);
	menu->AddSeparatorItem();

	BmMenuController* statusMenu = new BmMenuController(MENU_MARK_AS, target,
		new BMessage(BMM_MARK_AS), &BmGuiRosterBase::RebuildStatusMenu, BM_MC_MOVE_RIGHT);
	if (isContextMenu)
	...
	BmMenuController* moveMenu = new BmMenuController(MENU_MOVE, target, new BMessage(BMM_MOVE),
		&BmGuiRosterBase::RebuildFolderMenu, BM_MC_SKIP_FIRST_LEVEL | BM_MC_MOVE_RIGHT);
	if (isContextMenu)
	...
	AddItemToMenu(menu,
		CreateMenuItem("Filter (applies associated chain)", new BMessage(BMM_FILTER), "Filter"),
		target);

	BmMenuController* filterMenu = new BmMenuController(MENU_FILTER, target,
		new BMessage(BMM_FILTER), &BmGuiRosterBase::RebuildFilterMenu, BM_MC_MOVE_RIGHT);
	if (isContextMenu)
		filterMenu->SetFont(&font);
	menu->AddItem(filterMenu);
	AddItemToMenu(menu,
		CreateMenuItem("Create filter from mail" B_UTF8_ELLIPSIS, new BMessage(BMM_CREATE_FILTER)),
		target);

	menu->AddSeparatorItem();

	AddItemToMenu(menu, CreateMenuItem("Edit as new" B_UTF8_ELLIPSIS, BMM_EDIT_AS_NEW), target);
	menu->AddSeparatorItem();

	if (isContextMenu) {
		AddItemToMenu(menu,
			CreateMenuItem("Print" B_UTF8_ELLIPSIS, BMM_PRINT, "Print message" B_UTF8_ELLIPSIS),
			target);
		menu->AddSeparatorItem();
	} else {
		AddItemToMenu(menu, CreateMenuItem("Previous message", BMM_PREVIOUS_MESSAGE), target);
		AddItemToMenu(menu, CreateMenuItem("Next message", BMM_NEXT_MESSAGE), target);
		menu->AddSeparatorItem();
	}
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::PopulateLabelViewMenu(BMenu* menu)
{
	BMenuItem* item = new BMenuItem("Connect layout to parent", new BMessage(BMM_CONNECT_LAYOUT));
...
\*------------------------------------------------------------------------------*/
void
BmMailRefView::ShowMenu(BPoint point)
{
