...
bool
BmMainWindow::IsAlive()
{
	return BmMainWindow::nIsAlive;
}

void
UpdateAccounts(BmToolbarButton* button)
{
	...
	for (iter = TheRecvAccountList->begin(); iter != TheRecvAccountList->end(); ++iter) {
		...
		msg->AddString(BmRecvAccountList::MSG_ITEMKEY, iter->second->Key().String());
...
BmMainWindow::CreateInstance()
{
	if (!theInstance) {
		...
		TheMainWindow->SetWorkspaces(
			wspc == "Current" ? B_CURRENT_WORKSPACE : 1 << (atoi(wspc.String()) - 1));
...
BmMainWindow::BmMainWindow()
	: inherited("MainWindow", BRect(50, 50, 800, 600), beamApp->BmAppName.String(),
		  ThePrefs->GetBool("UseDocumentResizer", true) ? B_DOCUMENT_WINDOW_LOOK
														: B_TITLED_WINDOW_LOOK,
		  B_NORMAL_WINDOW_FEEL, B_ASYNCHRONOUS_CONTROLS | B_QUIT_ON_WINDOW_CLOSE),
	  ...
	  mErrLogWin(NULL)
{
	...
	MView* mOuterGroup = new VGroup(minimax(600, 200, 1E5, 1E5), CreateMenu(),
		new BmToolbar(new HGroup(minimax(-1, -1, 1E5, -1),
			mCheckButton = new BmToolbarButton("Check", width, height, new BMessage(BMM_CHECK_MAIL),
				this, "Check for new mail", true),
			...
			mSpamButton = new BmToolbarButton("Spam", width, height,
				new BMessage(BMM_LEARN_AS_SPAM), this, "Treat selected mail(s) as Spam"),
			mTofuButton = new BmToolbarButton("Tofu", width, height,
				new BMessage(BMM_LEARN_AS_TOFU), this, "Treat selected mail(s) as Tofu"),
			mPrintButton = new BmToolbarButton(
				"Print", width, height, new BMessage(BMM_PRINT), this, "Print selected mail(s)"),
			mTrashButton = new BmToolbarButton("Trash", width, height, new BMessage(BMM_TRASH),
				this, "Move selected mail(s) to Trash"),
			new BmToolbarSpace(), 0)),
		new HGroup(
			mVertSplitter = new UserResizeSplitView(
				new BetterScrollView(minimax(50, 100, 1E5, 1E5),
				...
				mHorzSplitter = new UserResizeSplitView(
					new VGroup(
						new HGroup(mMailRefViewFilterControl = new BmMailRefViewFilterControl(),
							new Space(minimax(0, 0, 1E5, 1E5, 0.3f)),
							mMailRefFilterControl = new BmMailRefFilterControl(), 0),
						new BetterScrollView(minimax(300, 50, 1E5, 1E5),
							...
							"99999 messages"),
						0),
					new BmMailViewContainer(minimax(300, 80, 1E5, 1E5),
						mMailView = BmMailView::CreateInstance(BRect(0, 0, 400, 200), false)),
					"hsplitter", 200, B_HORIZONTAL, true, true, true, true, false, B_FOLLOW_NONE),
				"vsplitter", 220, B_VERTICAL, true, true, true, false, false, B_FOLLOW_NONE),
			0),
		0);
...
BmMainWindow::CreateMenu()
{
	...
	menu->AddItem(CreateMenuItem(
		"Print message(s)" B_UTF8_ELLIPSIS, BMM_PRINT, "Print message" B_UTF8_ELLIPSIS));
	menu->AddSeparatorItem();
	AddItemToMenu(menu, CreateMenuItem("Preferences" B_UTF8_ELLIPSIS, BMM_PREFERENCES), beamApp);
	menu->AddSeparatorItem();
	menu->AddItem(new BmMenuController("Show log", this, new BMessage(BMM_SHOW_LOGFILE),
	...
	mAccountMenu = new BmMenuController("Check mail for", this, new BMessage(BMM_CHECK_MAIL),
		&BmGuiRosterBase::RebuildRecvAccountMenu, BM_MC_MOVE_RIGHT);
	menu->AddItem(mAccountMenu);
	menu->AddItem(CreateMenuItem("Check all accounts", BMM_CHECK_ALL));
	menu->AddSeparatorItem();
	subMenu = new BmMenuController("Send pending messages for", this,
		new BMessage(BMM_SEND_PENDING), &BmGuiRosterBase::RebuildSmtpAccountMenu, BM_MC_MOVE_RIGHT);
	menu->AddItem(subMenu);
	menu->AddItem(CreateMenuItem("Send all pending messages", BMM_SEND_PENDING));
	mMainMenuBar->AddItem(menu);
...
\*------------------------------------------------------------------------------*/
status_t
BmMainWindow::ArchiveState(BMessage* archive) const
{
...
\*------------------------------------------------------------------------------*/
status_t
BmMainWindow::UnarchiveState(BMessage* archive)
{
...
BmMainWindow::BeginLife()
{
	...
	try {
		...
		mErrLogWin = BmLogWindow::CreateAndStartInstanceFor("Errors", true, true);
...
\*------------------------------------------------------------------------------*/
void
BmMainWindow::WorkspacesChanged(uint32, uint32 newWorkspaces)
{
...
\*------------------------------------------------------------------------------*/
void
BmMainWindow::MessageReceived(BMessage* msg)
{
	try {
		...
		switch (msg->what) {
			...
			case BMM_LEARN_AS_TOFU:
			{
				BMessage tmpMsg(BM_JOBWIN_FILTER);
				mMailRefView->AddSelectedRefsToMsg(&tmpMsg);
				BmString jobName = (msg->what == BMM_LEARN_AS_SPAM)
				...
				switch (msg->FindInt32(B_OBSERVE_WHAT_CHANGE)) {
					case BM_NTFY_MAILFOLDER_SELECTION:
					{
						MailFolderSelectionChanged(
							msg->FindBool(BmMailFolderView::MSG_HAVE_SELECTED_FOLDER));
						break;
					}
...
\*------------------------------------------------------------------------------*/
void
BmMainWindow::Minimize(bool minimize)
{
...
\*------------------------------------------------------------------------------*/
void
BmMainWindow::MailFolderSelectionChanged(bool haveSelectedFolder)
{
	// adjust menu:
...
\*------------------------------------------------------------------------------*/
void
BmMainWindow::MailRefSelectionChanged(bool haveSelectedRef)
{
	// adjust buttons:
	...
	// adjust menu:
	mMainMenuBar->FindItem("Reply")->SetEnabled(haveSelectedRef);
	mMainMenuBar->FindItem("Forward")->SetEnabled(haveSelectedRef);
	mMainMenuBar->FindItem(BMM_REDIRECT)->SetEnabled(haveSelectedRef);
	mMainMenuBar->FindItem(BMM_EDIT_AS_NEW)->SetEnabled(haveSelectedRef);
...
\*------------------------------------------------------------------------------*/
void
BmMainWindow::MailViewChanged(bool hasMail)
{
	// adjust menu:
